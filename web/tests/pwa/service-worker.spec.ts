...
test.describe("Service Worker", () => {
  test("registers and activates on first load @smoke", async ({ page }) => {
    ...
    if (!controlled) {
      // SW may still be activating — check registration at least succeeded
      const registered = await page.evaluate(async () => {
        const reg = await navigator.serviceWorker.getRegistration("/");
        return reg !== undefined;
      });
      expect(registered, "Service worker should be registered").toBe(true);
      return;
    }
  ...
  test("static assets served from cache on second visit @smoke", async ({
    ...
    // Use a fresh context so we start with no cache
    ...
    await page
      ...
      .catch(() => {
        // SW may not control page until next navigation — that's OK
      });
